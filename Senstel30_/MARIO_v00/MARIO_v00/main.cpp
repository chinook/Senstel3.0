...
//SD card
SDBlockDevice bd(PC_12, PC_11, PC_10, PD_2);
FATFileSystem fs("sd");
...
// Writes a string to a log file to store acquisition data
int write_sd(string line, int run, string file = "/sd/log.csv")
{
	if (!fs.mount(&bd))
	{
		ofstream fos(file, ofstream::out);
		if (fos)
		{
			//printf("%s\r\n", line.c_str());
			fos.write((const char*)line.c_str(), line.length());
			fos.close();
		}
		else
			return 1;
...
// Writes a string to a log file to store acquisition data
int write_sd(struct SensorData *sdata, int run, string file = "/sd/log.csv")
{
	string s = to_string(run) + ',' + to_string(sdata->torque) + ',' + to_string(sdata->loadcell) + ',' + 
			   to_string(sdata->pitch) + ',' + to_string(sdata->wind_speed) + ',' + to_string(sdata->wind_direction) + 
			   to_string(sdata->rpm_rotor) + ',' + to_string(sdata->rpm_wheels) + ',' + to_string(sdata->angle_mat) + ','
			   + to_string(sdata->gear);
	...
	if (!fs.mount(&bd))
	{
		...
		{
			//printf("%s\r\n", line.c_str());
			fos.write((const char*)s.c_str(), s.length());
			fos.close();
		}
		else
			return 1;
